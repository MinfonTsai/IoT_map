$(function() {
	...
	//var redIcon = new tinyIcon({ iconUrl: '../assets/bicycle1-red.png' });
	var redIcon = new tinyIcon({ iconUrl: '../assets/orangeusb.png' });
        var yellowIcon = new tinyIcon({ iconUrl: '../assets/orangeusb.png' });
	//var yellowIcon = new tinyIcon({ iconUrl: '../assets/bicycle1-green.png' });
	...
	var lat_0=0,lng_0=0,acr_0=-1;
	var nStart=1;
	socket.on('load:coords', function(data) {
		if (nStart==1)//acr_0 == -1)
		{
			lat_0 = data.coords[0].lat;
			lng_0 = data.coords[0].lng;
			acr_0 = 16; //data.coords[0].acr;
			addrdata = data.coords[0].addr;
			showMap(lat_0, lng_0, acr_0);
		}
		if ((nStart == 2) && (data.id == "1122334455"))
		{
//console.log(data.id);
//console.log(data);
		//	map.setView([data.coords[0].lat,data.coords[0].lng],data.coords[0].acr);
			nStart = 0;
		}
...
console.log("map");
	...
	function setMarker(data) {
		for (var i = 0; i < data.coords.length; i++) {
			...
			//marker.bindPopup(addrdata).openPopup();	
	...
	setInterval(function() {
                for (var ident in connects){
                        if ($.now() - connects[ident].updated > 2000) {
				
//				markers[ident].bindPopup(addrdata);
//				markers[ident].openPopup();
                	}
			if ($.now() - connects[ident].updated > 6000) {//15000) {
